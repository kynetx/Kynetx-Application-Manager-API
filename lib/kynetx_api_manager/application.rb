...
#     @application_id = app_info["appid"]
#     @versions = app_info["versions"]
#     @valid = app_info["valid"]
#     @name = app_info["name"]
#     
#     # @source = @api.get_app_source(@application_id, @version,:hybrid)
#     #       
#     #         kybrid = JSON.parse(@source)
#     #         @meta = kybrid["metakrl"].join("\n") if kybrid["metakrl"]
#     #         @dispatch = kybrid["dispatchkrl"].join("\n") if kybrid["dispatchkrl"]
#     #         @global = kybrid["globalkrl"].join("\n") if kybrid["globalkrl"]
#     # 
#     #         kybrid["rules"].each do |rule|
#     #           a_rule = Rule.new
#     #           a_rule.name = rule["name"]
#     #           a_rule.status = rule["state"]
#     #           a_rule.code = rule["krl"].join("\n")
#     #           @rules << a_rule
#     #         end
#   rescue
#     raise "API failed to return predictable results: #{$!}"
#   end
# end